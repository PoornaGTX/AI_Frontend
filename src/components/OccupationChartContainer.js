...
import BarChart from "./OccupationBarChart";
import AreaChart from "./OccupationAreaChart";
import { useAppContext } from "../context/appContext";
import Wrapper from "../assets/wrappers/ChartsContainer";
import { generatallPDF } from "../utils/PDF_Occupation";

import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import "jspdf-autotable";

const OccupationChartContainer = ({ inputData, value, ageGroup, year }) => {
  const [barChart, setBarChart] = useState(true);
  const {
    recMonthlyApplications: data,
    occupationDeathCount,
  ...
  const newInputData = [
    ...inputData,
    { Year: year, DeathCount: occupationDeathCount },
  ];

  const tableData = newInputData.map((item) => ({
    AgeGroup: ageGroup,
    "Occupation Level": value,
    ...item,
  }));
  ...
  const generatePDF = async () => {
    ...
    const topSentence = `Occupation Level : ${value}\nPredcited Year : ${year}\nAge Group : ${ageGroup.replace(
      "-",
      " - "
    ...
    pdf.save(`${value}_predicition_report.pdf`);
  };
  ...
  return (
    <Wrapper>
      ...
      <h5>
        Occupation Level : {value} | Age Group : {ageGroup} | Year : {year}
      </h5>
