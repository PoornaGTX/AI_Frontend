...
const ReasonHistoryPage = ({ _id, year, age, reasonAllDeathCount, type, reasonvalue, GenratedDate }) => {
  const { setDeleteUser, setUpdateUser, user, deletePDF } = useAppContext();
    ...
    <Wrapper>
      ...
      <div className="content">
        <div className="content-center">
          ...
          <AdminUserInfo icon={<ImStatsDots />} text={`Prediction Year : ${year}`} />
          <AdminUserInfo icon={<ImUser />} text={`Age Group : ${agevalue}`} />
          <AdminUserInfo icon={<ImEye />} text={type === 'Reason' ? `Reason : ${reasonValueString}` : `Method : ${reasonValueString}`} />
        </div>
